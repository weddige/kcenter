...
def _step_size(shape, max_resolution):
    ...
    step = max((x_max - x_min) / max_resolution, (y_max - y_min) / max_resolution)
    plus_one = math.ceil(step)

    return x_min, x_max + plus_one, y_min, y_max + plus_one, step

def _grid_iter(x_min, x_max, y_min, y_max, step):
    x = x_min
    while x <= x_max:
        y = y_min
        while y <= y_max:
...
def grid_approximation(k, shape, max_resolution=10, epsilon=0.1):
    """This function approximates a shape covering by maximal max_resolution**2 grid points.

    :param k: int
    :param shape: BaseGeometry
    :param max_resolution: float
    :param epsilon: float
    :return: list of points
    """
    # Calculate grid points
    grid = []
    step = _step_size(shape, max_resolution)
